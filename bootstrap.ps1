...
if ($PackageLevel -in @("personal", "hardware-specific")) {
    ...
    Invoke-Expression "& { $(Invoke-WebRequest -useb https://gist.githubusercontent.com/Balfa/3a7e049dc1ac2d8ae1441a6e4e7c3f37/raw/OpenWithSublimeText.bat) }"
}
...
if ($PackageLevel -eq "hardware-specific") {
    ...
    Invoke-Expression "& { $(Invoke-WebRequest -useb https://raw.githubusercontent.com/Balfa/RdpPassToLocal/main/install.cmd) }"
    # And add soundswitch to startup
    Invoke-Expression "& { $(Invoke-WebRequest -useb https://raw.githubusercontent.com/Balfa/RdpPassToLocal/main/add-soundswitch-to-startup.cmd) }"
}
